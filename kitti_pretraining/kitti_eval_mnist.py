...
def run_eval():
    """
    Evaluate the KITTI-trained model (no finetuning) on MNIST data.
    Returns a tuple (avg_mse, avg_snr, avg_ssim).
    """
    # --------------------
    # Configuration
    # --------------------
    ...
    with open(txt_path, 'w') as f:
        ...
        f.write(f"MSE_STD: {std_mse:.6f}\n")
        f.write(f"SNR: {avg_snr:.4f}\n")
        f.write(f"SNR_STD: {std_snr:.4f}\n")
        f.write(f"SSIM: {avg_ssim:.4f}\n")
        f.write(f"SSIM_STD: {std_ssim:.4f}\n")
        f.write("-" * 50 + "\n")
    ...
    return (avg_mse, avg_snr, avg_ssim, std_mse, std_snr, std_ssim)
